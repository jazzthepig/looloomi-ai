...
- **锚点判据**(S-107):**好锚点按累积交付,不按跳跃** —— 判据 = 最好 10 天占比 + 正收益天数占比(funding 14.9%/73.6% vs 动量 152%/50.0%)。**先过这关再做收益检验。**
- **幸存者偏差 = 25.1pp/年**(S-111),是我们追过最大效应的 8 倍。`exchangeInfo` 的 `SETTLING` 是免费的死亡样本。**投资域准入 ADV>~$15M;再挑剔买不到东西**(S-112)。
...
## 数据法则(位置与陷阱,不记行数)
- **回测读 `ohlcv_daily_canonical`**(带 `bar_convention` / `open_usable`)。
...
- S-101/103/105:事件计数 + 中性化 + 正确基准后**五档 |t| 全 <2**;中位持有 2 天 ⇒ 换手成本 > 效应。
