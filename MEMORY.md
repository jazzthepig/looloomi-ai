...
- ~~①从未建过~~ **四层已填满**:① M-93 · ② M-87 · ③ M-88 · ④ R70/R71(数字在 PROJECT_STATE)。R76–R94 连败是④先做的设定错误。
- **下"不能声称"之前先 grep `_reports/` 说出该家族最新状态**(S-255):同一误读 08-19/08-27 各一次。**单向怀疑不是严谨。**
...
- **默认值越接近多数类越查不出 —— 危害与可发现性成反比**(S-122)。
- **守卫必须观测真实制品**(硬编码断言 / 静态 /health / 只进台账的教训 = 带绿勾的散文)。**永远在响的 warning 不携带信息**。**写了 ≠ 被执行**(S-244)。
- **按天加权的 alpha/t 在事件计数前不是证据,是叙事**(S-101:+7.99 → +3.58 t=1.55)。
- **中性化是读数前的单位换算,不是发表前的动作**(S-103):未中性化的 t 不是证据。
- **大额对消出的小数字比大数字危险**(S-103);**先判定假说是「连续关系」还是「稀有事件」再选检验**(S-108);**采样顺序自己控制时,先查该群体基准率 vs 总体**(S-112)。
...
- 决策链:`market_state_vectors`→`similar_market_states()`→`strategy_response`(`sample_grade='none'` = 一等信息)。
- pillar_O 是**异常探测器**;持续性 L/S 是误用。`cis_scores` 无市场数据列。
...
- **§STRATEGY-3 = R77 单策略**(R102, 2026-08-11):crypto micro-structure orthogonal 11/11 REFUTED ⇒ Lesson #54 path B locked.
