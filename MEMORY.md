...
- **锚点判据**(S-107):**好锚点按累积交付,不按跳跃** —— 判据 = 最好 10 天占比 + 正收益天数占比(funding 14.9%/73.6% vs 动量 152%/50%)。
- **幸存者偏差 = 25.1pp/年**(S-111),是最大效应的 8 倍;`SETTLING` 是免费死亡样本。**准入 ADV>~$15M**(S-112)。
- **`N_eff` 必须连同「约束哪本 book」一起引用**(S-115:长多 1.95 / 中性 3.31,都对),且是**窗口读数不是常数**(S-113)。
- **分散只能来自别的资产类**(S-114):加密内 ρ̄ **0.441** vs 加密↔TradFi **0.104**(62% 近零)。**纯加密只剩 ①+③。**
- **①层 book = `beta_core_nav`(2026-08-08 起),产品本体兼所有 book 的基准。**「超额」一律对 `benchmark_nav` 报;三本 L/S 已降级为研究记录。
...
- **持续期检验适用于「触发器」不只「持仓」**(S-117:`macro_regime` 中位 run **3 天**)⇒ **状态变量的中位 run 必须 ≥ 它驱动仓位的目标持有期**。
- **先判定假说是「连续关系」还是「稀有事件」再选检验**(S-108);**分组表必须把 n 印在同一行**。**自己控制过采样顺序时,第一行必须是该群体的基准率 vs 总体基准率**(S-112:85%→42% 的完美单调是回填顺序的产物)。

## 环境陷阱
- **监控分层按「频率」不按「资产数」**:日线@687 = 42MB/年(要宽,幸存者偏差靠它)· **小时线@687 = 1,096MB/年**(要窄)。**退市资产不订阅**;回填返回 **−2=未监控**,不可与 0 混同。
