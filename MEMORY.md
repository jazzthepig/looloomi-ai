...
- **锚点判据**(S-107):**好锚点按累积交付,不按跳跃** —— 判据 = 最好 10 天占比 + 正收益天数占比(funding 14.9%/73.6% vs 动量 152%/50.0%)。
- **幸存者偏差 = 25.1pp/年**(S-111),是我们追过最大效应的 8 倍。`SETTLING` 是免费的死亡样本。**准入 ADV>~$15M,再挑剔买不到东西**(S-112)。
- **`N_eff` 是窗口读数不是常数**(S-113:同一批 3.1/1.51),**且 `N/(1+(N−1)ρ̄)` 的等相关前提实测失效(离散度≈均值)⇒ 只当指示量**(S-114)。
- **分散只能来自别的资产类**(S-114):加密↔加密 ρ̄ **0.441**(1% 近零)vs 加密↔TradFi **0.104**(62% 近零)。**纯加密授权结构性只剩 ①+③。**
- **①层 book = `beta_core_nav`(2026-08-08 起 marking),它是产品本体,也是所有 book 的基准。**「超额」一律对 `benchmark_nav`(等权持有面板)报。三本 L/S book 已降级为研究记录。
...
- **先判定假说是「连续关系」还是「稀有事件」再选检验**(S-108);**分组表必须把 n 印在同一行**。**自己控制过采样顺序时,第一行必须是该群体的基准率 vs 总体基准率**(S-112:85%→42% 的完美单调是回填顺序的产物)。
...
- 决策链:`market_state_vectors`→`similar_market_states()`→`strategy_response`(`sample_grade='none'` = 从未出现过,一等信息)。锚点表 `funding_history`;回填游标步进绑 interval。
- `cis_scores` 无市场数据列 ⇒ 市场维须重建或记 NaN。pillar_O 是**异常探测器**,大多不触发 ⇒ 持续性 L/S 是误用。

## 已验证(引台账,不重推)
- R62 / S-103:`a_ret−b_ret` 是杠杆化 beta —— **同一个错误在档位聚合上重演过一次**。
