...
- **`N_eff` 必须连同「约束哪本 book」一起引用**(S-115):同一矩阵长多 **1.95**、中性 **3.31**,两个都对;且是**窗口读数不是常数**(S-113)。
...
- 决策链:`market_state_vectors`→`similar_market_states()`→`strategy_response`(`sample_grade='none'` = 从未出现过,一等信息)。回填游标步进绑 interval。
