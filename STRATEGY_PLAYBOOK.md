...
## Strategy ②-MOM — 面板内 动量 + 52 周高点 倾斜(Jazz 2026-09-26)— ⏳ 前向记录中,0/60 天

**层级:** ② β+,只做多满仓无杠杆,只倾斜不中性化。**因:** 趋势延续与「接近新高」的锚定效应在加密截面上长期存在(S-383 实测复合 CIS 的有效部分就是动量支柱)。
**账本:** `beta_plus_daily`,四臂:`panel_hold_w` / `momentum_52w_w`(周一信号、次日成交,主检验)与 `panel_hold_m` / `momentum_52w_m`(月初,低换手)。
**信号:** 排名均值( 动量组合[14/28/90 天], 52 周高点 );**权重** w=(1/N)(1+z),0~2/N。成本 10 bps;价格 binance_hist;起点 2026-09-25。
**历史(S-428,不是证据):** 周频 年化超额 +17.2%(t 3.22)、样本外 +10.0%、β 0.98、2020–2026 每年为正、任何 12 个月不输面板;
月频 +15.4% / 样本外 +7.0%、年换手 4.7×。**组合因子是看过 86 个变体后挑的,数值偏高。**
**判据(预注册):** 倾斜臂 − 同日程基准 的累计差,按 BTC 200 日线上/下分段报;≥60 个前向日才下结论。
