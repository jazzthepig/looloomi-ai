...
LIBRARY = [
    _rec("trend_v5c_long_only", notes="CAUSE: momentum-beta harvest of crypto up-cycles (behavioral trend persistence). NOT alpha — rides beta, defends crashes, bleeds chop (R49/R57).", regime={"trend": 1.9, "chop": -1.4, "risk_on": 2.1, "risk_off": -1.7},
    ...
    _rec("vol_carry_btc", notes="CAUSE: variance risk premium (crowd overpays for optionality in calm regimes). DISQUALIFIED on real option costs (R39).", regime={"calm": 2.7, "stormy": -1.5},
    ...
    _rec("funding_crowding_pooled", notes="CAUSE: leveraged-crowd funding extremes mean-revert (fear/greed recurrence). Market-neutral, dies in F1 quality-rotation regime (R47/R49/R60).", regime={"risk_on": -1.2, "risk_off": 1.1, "trend": 0.4, "chop": 0.9},
    ...
    _rec("cis_quality_ls_5d", notes="CAUSE: quality (pillar_O) price-discovery lag at 5d cadence — market slowly reprices on-chain health (R46/R62). Crypto-specific (R48); W5 late-cycle fragility.", regime={"risk_on": 1.2, "risk_off": 0.8, "trend": 1.0, "chop": 0.6},
    ...
    _rec("swing_overlay_v9", notes="CAUSE: execution alpha on 15m swing microstructure. PIT UNVERIFIED (4h→15m merge unaudited); capacity ~$2M (drag 17%/yr at $30M).", regime={"trend": 1.5, "chop": 0.3},
    ...
    _rec("ls_v4_ema_flip", notes="CAUSE: same trend persistence as v5c but churned 13x — momentum beta in an EMA costume, negative residual alpha (R49). Superseded.", regime={"trend": 1.2, "chop": -1.8},
    ...
    _rec("weekly_liquidity_gate", notes="CAUSE: marginal stablecoin-supply growth = liquidity inflow leads crypto beta (WEEKLY_REVIEW 2026-07-21). Single full-sample window; not gauntleted.", regime={"risk_on": 1.4, "risk_off": 0.9, "trend": 1.1},
    ...
    _rec("risk_direction_score", notes="CAUSE: smoothed multi-factor risk direction avoids binary whipsaw (R50*). Redundancy hub (0.90 to cis_quality) — little unique breadth.", regime={"risk_on": 0.5, "risk_off": 1.3, "chop": 0.2},
