"""
feather → Nautilus ParquetDataCatalog adapter for Sleeve A (Seth, 2026-07-17)
==============================================================================

Ports the same pattern as `ls_v1/data_adapter.py` but tailored for Sleeve A:
  - 1h bars (not 4h)
  - Source: `*_USDT_USDT-1h-futures.feather` files
  - Default feather dir: Sleeve E live data dir
  - Default catalog dir: /tmp/sleeve_a_catalog (ephemeral)

Parity target: `freqtrade CometCloudMultiFactorV2.py` on the same 3 pairs
(BTC/ETH/SOL) and the same 14mo window (2025-01-01 → 2026-03-12).
...
# Mirrors the freqtrade MultiFactorV2 config: BTC/ETH/SOL perpetuals,
# same precisions as the LS v1 catalog.
INSTRUMENTS = [
    ("BTC_USDT_USDT-1h-futures", "BTCUSDT-PERP", 1, 3),
    ("ETH_USDT_USDT-1h-futures", "ETHUSDT-PERP", 2, 3),
    ("SOL_USDT_USDT-1h-futures", "SOLUSDT-PERP", 3, 3),
]
...
def build_instrument(
    ...
    Fee levels mirror the freqtrade MultiFactorV2 config (0.05% taker,
    0.02% maker).  Margin init/maint 5%/2.5% — standard Binance isolated
    margin.  Same as LS v1 to keep the cross-sleeve parity uniform.
    """
...
def build_bars(df: pd.DataFrame, instrument: CryptoPerpetual) -> list[Bar]:
    """Convert dataframe rows to Nautilus Bar objects (1h).
...
def build_catalog(
    ...
    Idempotent: re-running wipes `instruments/` and `bars/` and rewrites
    from the source feathers.  This is intentional — stale rows in the
    catalog are the #1 source of "why does my backtest disagree with the
    freqtrade one" headaches.
    """
    ...
    for stem, symbol, price_prec, size_prec in INSTRUMENTS:
        ...
        logger.info(
            f"wrote {symbol}: {len(bars)} 1h bars "
