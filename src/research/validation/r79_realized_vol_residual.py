"""
R79 — Realized vol residual (cross-sectional demean of trailing realized vol) as
orthogonal candidate #3 (Seth, 2026-07-23).

Per R78 lesson #43 sharpens: orthogonal candidate screening can PASS the
leg-correlation gate but FAIL the gauntlet. R78 (relative momentum) was
orthogonal but had no standalone edge. R79 opens orthogonal candidate #3 on
the **microstructure-vol axis**: realized vol residual = σ[t, a] −
mean_a(σ[t, a]). Cross-sectional demean of trailing-N-day realized vol removes
the universe's common vol regime, leaving RELATIVE vol — which assets are MORE
volatile than the universe.

This is structurally different from:
  · R46 (CIS-quality multi-pillar rank)
  · R62 (absolute funding-z crowding)
  · R76 (funding residual — relative funding pressure)
  · R78 (relative momentum — relative trend strength)

Realized vol is a *microstructure* axis; the cross-sectional demean removes
the common vol-regime component, leaving the *idiosyncratic vol premium*.
Long assets with above-mean vol (high relative vol) is a documented
cross-sectional effect ("lottery preference"); short assets with below-mean vol
(low relative vol) is the carry equivalent.

R79 design (parallels R78):
  · Universe: same 28-asset strict funding ∩ CIS ∩ OHLCV (R46/R62/R73/R74/R76/R77/R78 parity).
  · Score: σ[t, a] − mean_a(σ[t, a]) — cross-sectional demean of trailing-30d
    realized vol (annualized, sqrt of sum of squared daily returns).
  ...
  · **Pre-test leg-correlation gate (lesson #42 anti-imposter)** — extended to
    4 existing fusion legs (R46, R62, R76, R78). If max |corr| > 0.30, flag
    as fusion-uncandidatable.
  · Both signs run; matched-cell sign verdict.

Verdict grammar:
  · ✅ SURVIVES + ORTHOGONAL — clears 3-check AND max |corr| ≤ 0.30 vs all 4
    existing legs. Eligible for fusion contribution (R80 candidate material).
  · 🟡 SURVIVES + CORRELATED — clears 3-check BUT correlated (|corr| > 0.30)
    with at least one existing leg → standalone-eligible but NOT fusion-
    candidatable.
  · 🔴 REFUTED — fails 3-check. Lesson #43 sharpens (R78 pattern): orthogonal
    candidates may not have any standalone edge after residualization; the
    fusion book may be near-optimal for this data on this universe.

Anti-imposter:
  - Realized vol residual is NOT the same as R46's CIS-quality (multi-pillar
    rank) nor R62's crowding-z nor R76's funding residual nor R78's relative
    momentum. The vol axis is *microstructure* — captures volatility-regime
    heterogeneity within the universe.
  ...
  - The R77 fusion-cell (R46+R62+R76 at w_R46=0.25, w_R62=0.75, w_R76=0.30) is
    FROZEN. R79 does NOT touch it.
  - R79 result informs a future R80 candidate (R79 as 4th fusion contribution)
    only if verdict is ✅ SURVIVES + ORTHOGONAL.
  - Per R78 lesson #43 sharpens: gate pass alone is NOT sufficient. R79 must
    ALSO clear the 3-check gauntlet to be fusion-candidatable.
"""
...
# R79-specific
R79_K_TERCILES = 3                    # R46 standard
R79_MIN_TRADEABLE = 12                # same floor as R73/R76/R78
R79_ORTHOGONALITY_GATE = 0.30         # lesson #42 — max |corr| vs existing legs
R79_VOL_LOOKBACK = 30                 # days; trailing realized vol window
R79_VOL_ANNUALIZATION = 365           # daily → annual: × sqrt(365)

# Sign constants (mirror R73/R76/R78 convention)
SIGN_HIGH_VOL_LONG = "high_vol_long"   # long assets with above-mean vol (lottery preference)
SIGN_LOW_VOL_LONG = "low_vol_long"     # long assets with below-mean vol (low-vol carry)
...
def score_realized_vol_residual(rets: pd.DataFrame, tradeable: list,
                                 lookback: int = R79_VOL_LOOKBACK,
                                 annualization: int = R79_VOL_ANNUALIZATION) -> pd.DataFrame:
    """Cross-sectionally demeaned realized vol: σ[t, a] − mean_a(σ[t, a]).

    σ[t, a] = sqrt(sum_{i=t-lookback+1..t} ret²_i) × sqrt(annualization)
              = annualized realized vol over the trailing `lookback` days.

    Cross-sectional demean at each t removes the universe's common vol-regime
    component (when ALL assets are more volatile, that's a market regime, not
    a relative signal); the residual is RELATIVE vol — which assets have
    above/below-mean vol on this date.

    Returns wide DataFrame (date × asset) on the tradeable subset. Warmup rows
    (where the lookback window isn't full) are NaN.
    """
    sub = rets[tradeable].copy().fillna(0.0)
    # Squared returns
    sq = sub ** 2
    # Trailing sum of squared returns (rolling window)
    trail_sq_sum = sq.rolling(window=lookback, min_periods=lookback).sum()
    # Annualized realized vol
    vol = np.sqrt(trail_sq_sum) * np.sqrt(annualization)
    # Cross-sectional demean at each t — only on fully-observed rows
    fully_observed = vol.dropna(how="any")
    demeaned_full = fully_observed.subtract(fully_observed.mean(axis=1), axis=0)
    # Reindex back to original index; warmup rows are NaN
    residual = demeaned_full.reindex(vol.index)
    return residual


# === L/S core (reuses R73's pillar_a_level_ls signature for parity) ==========
def realized_vol_residual_ls(score_wide: pd.DataFrame, rets: pd.DataFrame,
                              k_terciles: int = R79_K_TERCILES,
                              cost_bps: float = 0.0,
                              rebal_days: int = 1,
    ...
    (realized vol residual vs pillar_A level) but the L/S logic is the same:
    long top tercile, short bottom tercile, optional sign flip.
    """
...
def leg_correlation_gate_n(leg_new: pd.Series, existing_legs: dict,
                            gate: float = R79_ORTHOGONALITY_GATE) -> dict:
...
def _build_r62_detector_local(features: pd.DataFrame, fragile_mask: pd.Series,
    ...
    """Reproduce R62 best-cell detector on the R79 panel."""
...
def run(out_dir: Path,
    ...
    print(f"=== R79 — Realized vol residual (σ cross-sectional demean) L/S "
          f"(sign={sign}, k={R79_K_TERCILES}) ===\n")
    ...
    if len(tradeable) < R79_MIN_TRADEABLE:
        raise RuntimeError(
            f"Universe too small: {len(tradeable)} < {R79_MIN_TRADEABLE} "
            f"(R79_MIN_TRADEABLE floor). R79 refuses to silently widen.")

    # ── Score: realized vol residual ──────────────────────────────────────────
    print("Computing realized vol residual (σ cross-sectional demean) …")
    ...
    print(f"  Score shape: {score_volres_wide.shape}, "
          f"mean (post-warmup) = {score_volres_wide.mean().mean():.6f} "
          f"(should be ~0 by construction), "
          f"std (post-warmup) = {score_volres_wide.std().mean():.6f}")
    ...
    # ── Build R79 leg at default cadence (3d/0bps — mirrors R73/R76/R78 best cell) ─
    best_cad = 3
    leg_r79 = realized_vol_residual_ls(score_volres_wide, rets[tradeable],
                                          k_terciles=R79_K_TERCILES, cost_bps=0.0,
                                          rebal_days=best_cad, sign=sign)
    leg_r79 = leg_r79.reindex(rets.index).fillna(0.0)

    # ── Reproduce R46 + R62 + R76 + R78 legs on the same panel (gate prerequisites) ─
    ...
    print("Reproducing R78 leg (relative momentum 3d/0bps) for correlation gate …")
    ...
    gate = leg_correlation_gate_n(leg_r79, existing_legs)
    print(f"corr(R79_leg, R46_leg) = {gate['corr_new_vs_r46']:+.3f}")
    print(f"corr(R79_leg, R62_leg) = {gate['corr_new_vs_r62']:+.3f}")
    print(f"corr(R79_leg, R76_leg) = {gate['corr_new_vs_r76']:+.3f}")
    print(f"corr(R79_leg, R78_leg) = {gate['corr_new_vs_r78']:+.3f}")
    ...
    # ── Per-leg gauntlet ────────────────────────────────────────────────────
    g_r79 = gauntlet_3check(leg_r79.values, known_full, cut)
    ...
    print(f"Leg R79 ({best_cad}d/0bps): gross_t={g_r79['gross_t']:+.2f}, "
          f"OOS_t={g_r79['oos_t']:+.2f}, pass_all={g_r79['passes_all']}")
    ...
    for cad in cadences:
        for bps in cost_grid:
            fac_hi = realized_vol_residual_ls(score_volres_wide, rets[tradeable],
                                                k_terciles=R79_K_TERCILES, cost_bps=bps,
            ...
            fac_lo = realized_vol_residual_ls(score_volres_wide, rets[tradeable],
                                                k_terciles=R79_K_TERCILES, cost_bps=bps,
    ...
    if not viable_hi and not viable_lo:
        print("⚠ NO cell passes 3-check — R79 likely REFUTED.\n")
    else:
        print()

    # ── Final verdict ────────────────────────────────────────────────────────
    passes_3check = g_r79["passes_all"]
    orthogonal = gate["passes_orthogonality_gate"]
    if passes_3check and orthogonal:
        verdict = "✅ SURVIVES + ORTHOGONAL — R79 (realized vol residual) eligible as fusion contribution"
        ...
        verdict = ("🔴 REFUTED — fails 3-check; lesson #43 sharpens (R78 pattern): orthogonal "
                   "candidates may not have any standalone edge on this data")
    ...
    dd_r79 = max_drawdown(leg_r79)
    pw_r79 = per_window(leg_r79, windows)
    print(f"R79 maxDD: {dd_r79:+.2%}")
    print(f"R79 per-window (best-cad {best_cad}d/0bps, sign={sign}):")
    for label_, ann_pct, n in [(lab, pw_r79[lab]["ann_pct"], pw_r79[lab]["n_days"]) for lab in sorted(pw_r79.keys())]:
    ...
    out = {
        ...
        "construction": {
            "score_basis": "realized_vol_residual (annualized σ cross-sectional demean)",
            "vol_lookback": R79_VOL_LOOKBACK,
            "vol_annualization": R79_VOL_ANNUALIZATION,
            "k_terciles": R79_K_TERCILES,
        ...
        "per_leg_gauntlet": {
            "leg_r79": {"gauntlet": g_r79, "default_cad": best_cad, "default_cost_bps": 0.0,
                        "max_dd": dd_r79, "per_window": pw_r79},
        ...
        "live_book_impact": {
            ...
            "note": "R79 is research-only. R80 = R79 as 4th fusion contribution is the "
...
def format_report(payload: dict) -> str:
    """Human-readable R79 report."""
    lines = []
    lines.append("# R79 — Realized vol residual (σ cross-sectional demean) L/S")
    ...
    lines.append(f"- corr(R79, R46) = {g['corr_new_vs_r46']:+.3f}")
    lines.append(f"- corr(R79, R62) = {g['corr_new_vs_r62']:+.3f}")
    lines.append(f"- corr(R79, R76) = {g['corr_new_vs_r76']:+.3f}")
    lines.append(f"- corr(R79, R78) = {g['corr_new_vs_r78']:+.3f}")
    ...
    for leg in ("leg_r79", "leg_r46", "leg_r62", "leg_r76", "leg_r78"):
    ...
    if v["band"] == "SURVIVES_ORTHOGONAL":
        lines.append("- ✅ Lesson #43 reinforced across 3 orthogonal candidates: \"The "
                     "realized vol residual (σ cross-sectional demean) on the "
                     "microstructure-vol axis is structurally orthogonal to R46 (rank), "
                     "R62 (crowding-z), R76 (funding residual), and R78 (momentum residual). "
                     "R80 = R79 as 4th fusion contribution is the natural next step.\"")
    elif v["band"] == "SURVIVES_CORRELATED":
        lines.append("- 🟡 Lesson #43 sharpens: \"R79 standalone edge exists but it correlates "
                     "with at least one existing leg — likely R46 (volatility factor overlap "
                     "with multi-pillar rank) or R76 (volatility × funding cross-product). "
                     "Orthogonal candidate screening must catch this BEFORE gauntlet testing, "
                     "not after. R80 not warranted as fusion contribution.\"")
    else:
        lines.append("- 🔴 Lesson #43 sharpens (R78 pattern repeats): \"Realized vol residual "
                     "fails 3-check on this data. After R78 (momentum residual) and now R79 "
                     "(vol residual), the pattern is clear: orthogonal candidates on the "
                     "trend- or vol-axis do not have standalone edge in this universe. "
                     "R46+R62+R76 fusion may be near-optimal; future R-numbers should look "
                     "at carry-type orthogonal sources (term structure, basis) or "
                     "cross-asset / inter-class signals, not just demean'd single-class factors.\"")
    return "\n".join(lines)
...
if __name__ == "__main__":
    ...
    out = args.out_dir or Path(f"reports/r79_realized_vol_residual/{today}")
