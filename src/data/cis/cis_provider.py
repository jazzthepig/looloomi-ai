...
#: 评级带的下界,由高到低。`display_score()` 需要它来保证显示值不越带。
GRADE_FLOORS: tuple[tuple[str, float], ...] = (
    ("A+", 85.0), ("A", 75.0), ("B+", 65.0), ("B", 55.0),
    ("C+", 45.0), ("C", 35.0), ("D", 25.0),
)
...
def display_score(raw: float, dp: int = 1) -> float:
    ...
    实测 2026-08-27 的排行榜首屏:
        ...
        Aave  75.7  A
        Uniswap 75.0  B+      ← 看起来像 bug,因为它就是

    `UNI` 的 `raw_cis_score = 74.97`。`get_grade(74.97)` 给 B+ —— **正确**,
    74.97 < 75。而显示值是 `round(74.97, 1) = 75.0`,**四舍五入跨过了 75 这条
    grade 自己遵守的线**。于是同一行里,数字说 A 档,徽章说 B+ 档。

    这是今天那个形状的又一个位置:**同一个量的两个表示,在一条决策边界上分了岔。**
    而这次它出现在产品首屏,任何一个 LP 打开就能看见。

    修法不是把 grade 改成按显示值算 —— 那会让"看起来够了"变成"就是够了",
    等于让呈现层决定评级。反过来:**显示值向下让到本带内最大的那个 1 位小数。**
    74.97 → 74.9,和 B+ 一致;75.21 → 75.2,和 A 一致,不受影响。

    代价说清楚:落在带边界正下方的分数会显示得比四舍五入低 0.1。
    **那个 0.1 的代价,换的是数字与徽章永不矛盾** —— 而矛盾是要解释的,0.1 不用。

    ⚠️ 第一版查错了边界:我取的是**当前评级带的下界**(B+ → 65.0),
    而要防的是越过**上一带的下界**(A → 75.0),也就是本带的天花板。
    穷举 0.00–100.00 每 0.01 一个点,第一版留下 **34 处矛盾**,74.97 赫然在列 ——
    **也就是说它一个都没修。** 用穷举而不是几个手挑的例子,是它当场暴露的原因。
    """
    if raw is None:
        return raw
    x = float(raw)
    r = round(x, dp)
    # 本带的天花板 = 所有下界里,大于 x 的那个最小值。没有就说明已在最高带。
    ceiling = min((f for _g, f in GRADE_FLOORS if f > x), default=None)
    if ceiling is not None and r >= ceiling:
        # 四舍五入把它推进了上一带 —— 退到本带内最大的那个 dp 位小数
        return round(ceiling - 10 ** (-dp), dp)
    return r
