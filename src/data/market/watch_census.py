...
COVERAGE: dict[str, str] = {
    ...
    "corporate_treasury_history": "data-freshness:producers (S-293,状态快照)",
}
...
EXCLUDED_BY_DESIGN: dict[str, str] = {
    ...
    # ⚠️ S-331:这张表是我 2026-09-11 建的,建完当天就把「无判决对象」
    # 从 40/72 顶到 41/73 —— **我加了一个诊断工具,它自己立刻变成了一个
    # 没人判活的对象。** 这条排除是显式的,理由也写在这里:
    # 它是 `/internal/write-probe` 的一次性靶子,每次探针写完即删,
    # **0 行是它的正常状态**,而「它今天没被写」不构成任何关于系统的事实。
    # 它的 RLS 与授权刻意和 NAV 表一致,所以探针走的是真实权限路径 ——
    # 但那是为了让探针可信,不是为了让这张表值得判活。
    "_write_probe": "写路径探针的一次性靶子 —— 写完即删,0 行是正常状态",
