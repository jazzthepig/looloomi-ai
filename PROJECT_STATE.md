...
**Last updated:** 2026-09-08 (Seth/Cowork lane — S-296…S-323;**S-323 最贵的一条**:PostgREST 服务端 `db-max-rows`=1000 把 `limit=100000` 静默截断**并返回 200** —— 深盘 universe 库里 262、读出 **2**,而 `collect_deep_panel` 与 S-318 刚接上的 `_cg_panel_loop` 拿着 2 个照报 ok(**覆盖 2 个资产的成功和覆盖 262 个的成功输出完全一样**);`coverage_report` 同形,合格行 1759 截在 1000,**分子分母被同一次截断同时改写所以比值永远看着合理** —— 而这条教训 S-130 就写在 `beta_core_paper` 的 docstring 里:**知道一件事和把它编码进去是两个动作,只有第二个能活下来**;`_beat()` 漏转发 `detail` 令 `_forward_record_loop` 连续 7 轮 TypeError,守卫全程绿灯因为它里面写着一行豁免 —— **豁免记的是「当时不需要」,它不会自己过期**;S-322 `_fossil_note` 归一三个分支,**陈旧的 ok 比陈旧的 failing 更危险**(前者读作「已修好」→ 停止查);**S-323b — S-207 卡三周的根因就是这个截断**:深盘从 08-18 起每天只进 1–4 个标的(应 262)而 `max(trade_date)` 始终是当天,于是 `refresh_depth_divergence` 的 ≥97 覆盖地板**正确地**拒绝、`depth_divergence_log` 停在 08-23、Telegram 报 15 天没标记 —— **地板从来没错,是我喂给它的面板只有 2 个标的宽**;第一版修复又把「RPC 没通」和「库里是空的」折成一个 `[]`(**在修「两个状态一个表示」的补丁里又做了一次**)→ 改三值;`_DEFAULT_DAYS=14` 的重叠窗**同时是上限**,够不到 21 天的洞 → 改为按 `latest` 中位数自愈;**S-323c 真正的原因是没索引**:`ohlcv_daily`(46 万行)两个索引都不以 `source` 打头 → 按 source 分组全表扫,`deep_panel_symbol_list` 3476ms / `refresh_depth_divergence` 5518ms,而 anon/authenticated 的 `statement_timeout` 是 **8s** → 生产争 I/O 时越线 → PostgREST 5xx → 客户端报「**no response from Supabase after retries**」,**一句描述症状、藏起原因的话,把我挡在外面两轮**;加 `(source,symbol,trade_date)` 索引后 **181ms / 1367ms**;⚠️ 未修已记:`_supabase_request_with_retry` 把「熔断/超时/5xx耗尽」三态折成一个 None;⚠️ 仍缺:Sense 入口 24 · 无判决对象 40/72 · ②的 Learn 无数据)
