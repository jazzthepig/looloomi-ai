...
**Last updated:** 2026-09-04 (Seth/Cowork lane — **S-289:规则与它旁边的例子矛盾时,输的总是规则。** 三方对账:Minimax-C 的交易模块审计 8 条里 **3 条事实错**(表名不存在 ×2、「没有 book 在跑」实为 6 本在 mark)、1 条低估 4 倍,**且漏了两个真死的**(`two_layer` 13d · `pod_aggregator`/`factor_tilt` 各 0 行)—— 原因是他抽查时正逢 ① 熄火窗口且无 Supabase 读权限,S-276 同款。已建 `paper_book_freshness()` RPC + `/internal/data-freshness` 的 `books` 块(`never_wrote` 与 `stale` 刻意不合并)。摄入 lane 规则 3b 从散文变成 CI(判据是**写**不是**抓**)。**交接块反复出错的根因找到了:CLAUDE.md 的模板自己带着行尾注释** —— 规则在模板下面,而被复制的是模板;已守卫 CLAUDE.md 自己。 **同日自咬:** 给 `books` 加 token gate 时用 `if _tok_ok:` 包住整块 ⇒ 无 token 时字段不存在,与「没部署」同形;`column_drift: null`(旧构建)与 `{}`(干净)差一个字符意思相反。已改为永远出字段 + `checks:[tables,columns]` —— **回声必须说出它检查了什么,不只是发现了什么。** —— 前一轮:**S-287:「有没有价」≠「今天有没有价」。** `load_binance_panel` 把缺价用昨价 forward-fill,而 `px` 的入选条件只有 not-NaN 且 >0 —— **填出来的值两条全满足**,于是停更的 symbol 当作实时报价进书、计入 80% coverage 底线、照 mark。守卫问的是「这里有没有一个数」,问题是「这个数是不是今天观测到的」。**这是 S-194 的下面一层**(它分开了「没有数据/没有变动」,而下面一层仍混着「过期数据/数据」)—— 本周第五次「作用域差一格」。fill 不删(研究侧需要连续序列),改为**可见**:`with_fill_mask=True` 追加掩码、默认 arity 不变(14 个调用点;逼着全仓库改一遍的守卫会被回滚),① 排除最后一行被填充的 name,排除事件写 `nav_exceptions` **即使 mark 成功也写**。**仍只在 ① 上:`combined_book`/`scalable_paper` 同样吃 forward-fill 的价,且都还没接 `mark_coverage`。**  —— 前一轮:**S-286:声明 ≠ 执行。** `_VALUATION_POINT_TOLERANCE_MIN` 全生命周期**只有一个 reader:断言它被拼写了的那个测试** ⇒ v5 的 inception 行在 **07:00:52 UTC** 落下,偏离 00:05 估值点 **6h55m**,所有守卫全绿。根因:S-283 只把**尾部** sleep 指向墙钟,`mark 1`(每次部署后那次)没有 —— **循环的第一次迭代是一条路径。** 已修:容差真比较真拒绝 + 10 个循环在 `while True` 前等点。同时 `/internal/schema-drift` 只查表不查列(建于 S-166 的 11 张缺表,继承了那次的作用域),而昨天熄火正是缺一个列 —— 已加 `write_columns()` + `supabase_missing_columns()`,**现场核对 8 表 84 列 0 漂移**。**离线只能证明迁移文件存在,只有线上探针能证明迁移跑过。** 守卫被变异测试绕过四次后改为**行为测试**(注入 415 分钟偏离,断言拒绝且 `_load_panel` 未被调用)。 —— 前一轮:**S-285:① 账本上线即熄火,已恢复。** v5 部署跑在 migration 前面 ⇒ `interval_hours` 列不存在 ⇒ 每次 insert 被答 400 ⇒ `/api/v1/beta-core/curve` 返 `{"days": 0}`;而 `_write` 返 False 时**一行记录都不写**,于是**空曲线和「从没跑过」长得一模一样**。已补跑迁移(v4 12 行全部 voided · `nav_exceptions` 已建)、建 `_record_exception()` 接进 6 条拒绝路径、更正 NAV_POLICY §3(拒绝写 `nav_exceptions`,**不写 NAV 行** —— NAV 表里的一行断言「NAV 被 struck 过」)。**下一个估值点 00:05 UTC 09-05;在那之前 ① 曲线为空是预期,不是故障。** 另:`897861b` 把 2,379 行塞进一个覆盖 4 行的标题(规则 6),不重写历史,台账留更正。 —— 同轮 **S-283:NAV 读数三个 P0** 见 `docs/NAV_POLICY.md`。前一轮:**S-284:今日 8 个修补落地(H/I/J/K/O/F + B 修 + E 决策 ticket)。** quick-win batch(单 commit):`paper_trading/__init__.py` 填 `__version__` + `__all__` 16 项;`spec_runner.Decision.as_payload()` 加 `verdict_kind` 字段;`survivors_only_lag1_book_bookB` 退役,改单名 + 加载时拒绝;`daily_runner.py` 删 `if False` 死代码;`MIN_MEASURABLE` 30→12 + `why_hidden` JSON;`DISPLAY_SCORE_DP=1` 抽出常量 + 新单测。B 修:`/health.data_layer.regime_quorum` 暴露 `LAST_REGIME_QUORUM`,**空 dict = 未测量**(消费者须知)。S-283:NAV 读数三个 P0,全在 ① 这本 benchmark 账上。Jazz 问「为什么这周 beta 没捕捉到」→ beta 捕捉到了(excess 逐行 0.000000),错的是读数;他接着指出「读数读不准不是偶发的,很多基金都有类似问题」,于是补了整套 fund-admin 控制。① **v4 整段 VOID,v5 从 1.0 起步(代价 12 天 forward record)**;新增 `docs/NAV_POLICY.md` v1(13 条控制对照 SEC 2a-5 / AICPA / CSSF 24/856 / GIPS)+ `tests/test_nav_policy.py`(已进 preflight)+ migration。**⚠️ migration 必须先于代码上线**,否则 ① 账静默停止 marking。前一轮:S-262…S-283)
