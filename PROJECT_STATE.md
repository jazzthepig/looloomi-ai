...
**Last updated:** 2026-07-19→2026-07-20 (Minimax-B; §CIS-HISTORY-BACKFILL re-run complete: **R45 🔴 logged** — CIS-quality long/short L/S REFUTED at standard construction (composite survives gross in-sample +48.4%/yr t=+2.24 but degrades to t=+1.68 at 5 bps turnover, and OOS t=+0.33 on last 30%; pillar_O dominates in-sample t=+2.49/+51.4%/yr but flips to t=−0.45 OOS; **composite adds nothing over pillar_O at t=+0.44**). Ledger aggregate lesson #13 added: gross-in-sample + cost-failure + OOS-failure is the refutation pattern for factor sleeves — the three-check gauntlet (gross t > 1.96, cost-t > 1.96 @ 5 bps, OOS t > 1.96) belongs in every factor gauntlet. **Actionable CIS upgrade signal: reweight CIS v5 toward pillar_O (On-Chain/Health) and away from pillar_S (Sentiment, which is actively negative at α_t=−0.56).** Composite retains F/M/A as diversifying. The signal IS partially real (~+50%/yr gross in-sample uncorrelated to market+momentum) but not at tradable magnitude in this construction — try weekly/monthly rebalance or use composite CIS as quality/risk filter, not return predictor. Built `src/research/validation/cis_quality_absorption.py` (290 LoC) + date-resolution fix in `cis_quality_factor.py`; run artifacts at `reports/cis_quality_absorption/2026-07-19/{verdict.json,REPORT.md}`. Per Jazz's steer "有比 cis 策略更好的，我们就用，这样 cis 才会升级" — the better-of-the-two is pillar_O alone; CIS gets the upgrade. Previous state preserved below for context.)
