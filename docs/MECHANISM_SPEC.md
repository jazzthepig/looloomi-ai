...
| **Cross-sectional L/S, regime-gated with macro_regime tag (R47 proxy)** | 🟢 STRUCTURAL SURVIVE + OOS-CONFIRMED | R47 (18 cells = 2 pillars × 3 skip variants × 3 cadences; 14/18 cells clear +0.5 strategy-credit; best `R47_pillar_A_skip_all_bear_cad5` @ fwd β-adj SR **+1.183**, 4/4 folds positive, 5/6 sub-periods positive). R69 (216 cells = 4 sources × 6 pillar combos × 1 skip × 3 cadences × 3 costs; **43/216 cells clear +0.5**, **6/216 cells clear +1.0**, **29/216 cells have |IS t| > 1.96**; all 4 acceptance criteria pass; best cell `R69_smoothed_persistence_pillar_O_cad5_cost5` @ **+1.397**; replicator cell `R69_default_pillar_A_cad5_cost5` @ +1.183). R70 (72 cells held-out OOS 2026-01-08 → 2026-06-07; **5/72 cells clear +1.0**, **8/72 cells clear +0.5**; best cell `R70_default_pillar_A_cad3_cost5` @ **OOS β-adj SR +1.580**; survives 20 bps cost at +1.315; smoothed sources can't be tested in this window due to data range) | Aggregate lesson #14.4 + #14.5 + #14.6: (a) **regime proxy quality is load-bearing** (macro_regime tag breaks cross-sectional feedback loop); (b) **two distinct survivor patterns**: default + pillar_A (conservative, walk-forward clean) and smoothed_persistence + pillar_O (highest SR +1.397, less walk-forward robust); (c) **cad3 outperforms cad5 in the recent held-out window** — R47/R69's cad5 recommendation is replaced by cad3 for production. The macro_regime tag from the CIS JSON is a direct, hand-curated macro label (EASING / RISK_ON / RISK_OFF / TIGHTENING / STAGFLATION). **R70 confirms R69's finding survives held-out OOS testing on default source; cad3 is the new production-paper candidate.** |
